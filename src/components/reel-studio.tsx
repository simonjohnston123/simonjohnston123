...
export function ReelWizard({ locationId, priceLabel, onClose }: { locationId: string; priceLabel: string; onClose: () => void }) {
  ...
  function pickPresenter(id: string) {
    setPresenter(id); setStep(3); setScript(null); setMsg(null);
    start(async () => {
  ...
  function create() {
    ...
    start(async () => {
      const r = await createRenderJobAction({ locationId, productId: product.id, productName: product.name, presenter, script });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div onClick={(e) => e.stopPropagation()} className="relative z-10 max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl">
        ...
        {done ? (
          ...
          <div>
            <div className="mb-2 text-sm font-semibold text-slate-700">2 · Pick your presenter</div>
            <div className="grid gap-3 sm:grid-cols-2">
              {PRESENTERS.map((p) => (
                <button key={p.id} onClick={() => pickPresenter(p.id)} className="rounded-2xl border border-slate-200 p-4 text-left transition hover:ring-2 hover:ring-brand-400">
            ...
            {!script ? (
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between rounded-xl bg-slate-50 p-3">
                  <div className="text-sm text-slate-600">Presenter: <strong className="capitalize">{presenter}</strong> · ~30s vertical video</div>
